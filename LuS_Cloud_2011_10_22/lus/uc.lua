		...
		elseif self:IsBanned() then
			...
			if data.ip ~= nil and data.serialhash ~= nil then
				...
				if self.protection == 0 then
					if ipmatch == false or serialmatch == false then
						InputConsole("msg [LuS] Warning, %s might be a nickspoofer! (serialhash or IP doesn't match)", self:GetName())
			...
			if self.protection > 0 then
				...
				if valid == false then
					msg = string.format("[c=04][b][Protection][/b] Info: Account Protection triggered on [u]%s[/u]. Protection Level: [n]%d[/n] | Account IP: [u]%s[/u] | Account Serial: [u]%s[/u][/c]", self:GetName(), self.protection, self.accountip, self.accountserial)
					irc.SendMsg(msg)
