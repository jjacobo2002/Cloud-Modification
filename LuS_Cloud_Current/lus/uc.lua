...
function Player:Init(pID, Nick)
	...
	temp.dp = 0
	temp.loti = 0
	temp.interest = sc.interest
	temp.allow_interest = false
	temp.vip = 0
	temp.last_vip_update = 0
	temp.vip_subscribe = 0
	temp.vip_months = 0
	temp.access = 0
	temp.currentip = Get_IP_Address(pID)
	temp.accountip = nil
	temp.currentserial = 0
	temp.accountserial = nil
	temp.jointime = 0
	temp.last_present = 0
	temp.onlinetime = 0
	temp.onlinecounter = os.time()
	temp.mute = false
	temp.sound = 0
	temp.protection = 0
	temp.autologin = 0
	...
	temp.kills = 0
	temp.last_killed = 0
	temp.start_killspree = os.time()
	temp.lastkey = os.time()
	temp.modpower = false
	temp.bonuscreds = false
	temp.scripts = false
	temp.killsuit = false
	temp.absorbsuit = false
	temp.vipgamble = 0
	temp.zombiesuit = false
	temp.mrmoney = false
	temp.unstuck = false
	temp.lt_in_veh = 0
	...
	if self:IsDisabled() == false then
		...
		elseif self:IsBanned() then
			...
			if tonumber(data.count) < 1 then
				InputConsole("msg [LuS] Attention everyone! Please give a warm welcome to the newbie: %s!", self:GetName())
				
				sql = string.format("INSERT INTO users (nickname, jointime) VALUES('%s', %d)", self:GetName(), os.time())
				db.Query(sql)
				helpcmd.ShowNewb(self.pID)
			end
				
			sql = string.format("SELECT * FROM users WHERE nickname LIKE '%s' LIMIT 1", self:GetName())
			result = db.Query(sql)
			data = result[1]
			...
			if data.ip ~= nil and data.serialhash ~= nil then
				...
				if self.protection == 0 then
					if ipmatch == false and serialmatch == false then
						InputConsole("msg [LuS] Warning, %s might be a nickspoofer! (Serialhash and IP does not match account)", self:GetName())
						msg = string.format("[c=04][b][Protect][/b] Info: %s's Account IP: [u]%s[/u] | Account Serial: [u]%s[/u][/c]", self:GetName(), self.accountip, self.accountserial)
						irc.SendMsg(msg)
						msg = string.format("[c=04][b][Protect][/b] Info: %s's Current IP: [u]%s[/u] | Current Serial: [u]%s[/u][/c]", self:GetName(), self.currentip, self.currentserial)
						irc.SendMsg(msg)
					end
				end
			...
			if self.protection > 0 then
				...
				if valid == false then
					msg = string.format("[c=13][b][Protection][/b] Info: Account Protection triggered on [u]%s[/u]. Protection Level: [n]%d[/n] | Account IP: [u]%s[/u] | Account Serial: [u]%s[/u][/c]", self:GetName(), self.protection, self.accountip, self.accountserial)
					irc.SendMsg(msg)
					...
					msg = string.format("[c=03][b][Protect][/b] Info: %s has [u]passed[/u] protection level [u]%d[/u] and is [u]NOT[/u] a nickspoofer![/c]", self:GetName(), self.protection)
					irc.SendMsg(msg)
				end
			elseif data.password ~= nil then
				msg = string.format("[c=03][b][Protect][/b] Info: %s [u]is[/u] protected by a password (protection level [u]0[/u])[/c]", self:GetName())
				irc.SendMsg(msg)
			end
...
function Player:BDay()
	...
	elseif (playtime - self.last_present) > month then
		...
		passed = mf.TimePassed(self:GetOnlineTime(), 3)
		InputConsole("msg [LuS] %s has spent %d hours, %d minutes and %d seconds on this server since %s", self:GetName(), passed['hours'], passed['minutes'], passed['seconds'], mf.FormatDate(self:GetJoinTime()))
	end
end
...
function Player:SetBonusCreds(State)
	...
	if State == true then
		...
		Attach_Script(pObj, "JFW_Credit_Trickle", "5,1")
	else
...
function Player:SetKillsuit(State)
	if State == true then
		self.killsuit = true
	else
		self.killsuit = false
...
function Player:HasKillsuit()
	return self.killsuit
end

function Player:SetAbsorbsuit(State)
	if State == true then
		self.absorbsuit = true
	else
		self.absorbsuit = false
	end
end

function Player:HasAbsorbsuit()
	return self.absorbsuit
end

function Player:GetLastVipGamble()
	return self.vipgamble
end

function Player:SetLastVipGamble(sec)
	self.vipgamble = sec
end

function Player:SetZombiesuit(State)
	if State == true then
		self.zombiesuit = true
	else
		self.zombiesuit = false
	end
end

function Player:HasZombiesuit()
	return self.zombiesuit
end

function Player:IsMrMoney()
	return self.mrmoney
end

function Player:SetMrMoney(State)
	if State == true then
		self.mrmoney = true
	else
		self.mrmoney = false
...
function Player:SetUnstuck(State)
	if State == true then
		self.unstuck = true
	else
		self.unstuck = false
