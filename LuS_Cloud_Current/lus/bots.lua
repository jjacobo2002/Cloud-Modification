...
function Command(pID, Message)
	...
	if Message[1] == "!patchbot" or Message[1] == "!pbot" then
		if user[pID]:HasAccess(1) then
			if Get_Team(pID) == 1 then
				...
				if Get_Money(pID) < price then
					...
					MultiSpawn(Get_GameObj(pID), "GDI_Grenadier_2SF", 3, 3, 0.1, false)     
	...
	if Message[1] == "!lcgbot" or Message[1] == "!laserchainbot" then
		if user[pID]:HasAccess(1) then
			if Get_Team(pID) == 0 then
				...
				if Get_Money(pID) < price then
					...
					MultiSpawn(Get_GameObj(pID), "Nod_RocketSoldier_2SF_AutoRifle", 3, 2, 0.1, false)
