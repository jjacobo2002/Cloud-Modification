...
function Command(pID, Message)
		...
		if user[pID]:HasAccess(1) then
			...
			if Get_Preset_Name(Get_GameObj(pID)) == "CnC_GDI_Engineer_0" or Get_Preset_Name(Get_GameObj(pID)) == "CnC_Nod_Engineer_0" then
				...
				for _,Building in pairs(Get_All_Buildings()) do
					distance = mf.Get_Distance(pObj, Building, false)
					if distance < 10 then
	...
	if Message[1] == "!aqollo" then
		if user[pID]:HasAccess(1) then
			...
			if pc <= 4 then
				InputConsole("ppage %d You can't buy an aqollo when there's four or less players.", pID)
			elseif Get_Money(pID) < price then
				if Get_Team(pID) == 1 then
					InputConsole("sndp %d m00evag_dsgn0028i1evag_snd.wav", pID)
				else
					InputConsole("sndp %d m00evan_dsgn0024i1evan_snd.wav", pID)
	...
	if Message[1] == "!warmedic" then
		if user[pID]:HasAccess(1) then
			...
			if Get_Money(pID) < price then
				...
				InputConsole("msg %s has bought a war medic! They can instantly !heal a friendly character 100 hp/ 100 ap every minute!", user[pID]:GetName())

				user[pID].warmedic = true
	...
	if Message[1] == "!heal" then
		if user[pID]:HasAccess(1) then
			if user[pID].warmedic == true then
				wait_time = 60
				if user[pID].warmedic_time == nil then
					timeleft = wait_time - wait_time
				else
					timeleft = wait_time - (os.time() - user[pID].warmedic_time)
				end
				...
				if timeleft <= 0 then
					...
					elseif Get_Team(tID) ~= Get_Team(pID) then
						InputConsole("ppage %d That player is not on your team! Whadda thinkin!?!?", pID)
					else
						tObj = Get_GameObj(tID)

						Set_Shield_Strength(tObj, Get_Shield_Strength(tObj)+100)
						Set_Health(tObj, Get_Health(tObj)+100)

						InputConsole("ppage %d You have healed %s 100 health and 100 armor.", pID, user[tID]:GetName())
						InputConsole("ppage %d You have been healed 100 health and 100 armor by %s", tID, user[pID]:GetName())

						user[pID].warmedic_time = os.time()
					end
				else
					InputConsole("ppage %d You need to wait %d seconds to use !heal", pID, timeleft)
				end
			else
				InputConsole("ppage %d You are not a war medic.", pID)
			end
		else
			InputConsole("ppage %d You do not have access to this command.", pID)
		end
	...
	if Message[1] == "!refundme" then
		if user[pID]:HasAccess(1) then
			...
			for _,Building in pairs(Get_All_Buildings()) do
				local distance = mf.Get_Distance(Get_GameObj(pID), Building, false)
				if distance < 10 then -- less then 40 meters
					...
					if veh == 0 then
						...
						elseif Get_Preset_Name(Get_GameObj(pID)) == "civ_lab_tech_01" and Get_Model(Get_GameObj(pID)) == "dino" then
