...
function Receive(Channel, User, Message)
	...
	if string.find(Channel, "#.+") ~= nil and string.lower(Channel) ~= string.lower(Default_Channel) then
		Part(Channel)
		print(string.format("IRC Left channel: %s", Channel))
	elseif Channel == Nickname and User ~= Ignore then
...
function Callback(Packet)
	...
	if tok[2] == "PRIVMSG" or tok[2] == "PART" or tok[2] == "JOIN"  or tok[2] == "NICK" then
		...
		if tok[2] == "PRIVMSG" then
			Receive(channel, user, message)
		elseif tok[2] == "PART" and user ~= Nickname and user ~= Ignore and string.lower(channel) == string.lower(Default_Channel) then
			UserLeave(user)
		elseif tok[2] == "JOIN" and user ~= Nickname and user ~= Ignore and (string.lower(channel) == string.lower(Default_Channel) or string.lower(channel) == string.lower(":"..Default_Channel)) then
			UserJoin(user)
		elseif tok[2] == "NICK" and user ~= Nickname and user ~= Ignore and string.lower(channel) == string.lower(Default_Channel) then
