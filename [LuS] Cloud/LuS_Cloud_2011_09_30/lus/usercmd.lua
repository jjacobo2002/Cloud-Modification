	...
	if Message[1] == "!callmod" then
		if user[pID]:HasAccess(1) then
			if gi.GetModCount() > 0 then
				...
				if (os.time() - lastmodcall) > 60 then
					modcalled = 0
					for User, v in pairs(ircuser) do
						if ircuser[User]:HasAccess(2) then
							irc.SendPM(string.format("Player %s has requested for a mod.", user[pID]:GetName()), User)
					...
					InputConsole("ppage %d This command has already been used in the last minute, please be patient!", pID)
				end
			end
		else
			InputConsole("ppage %d You do not have access to this command.", pID)
