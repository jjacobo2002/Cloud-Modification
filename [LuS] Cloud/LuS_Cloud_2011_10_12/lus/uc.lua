			...
			if data.ip ~= nil and data.serialhash ~= nil then
				...
				if self.protection == 0 then
					...
					elseif serialmatch == false then
						...
						msg = string.format("[c=04][b][Protect][/b] Info: [u]%s[/u]'s Account Serial: [u]%s[/u][/c]", self:GetName(), self.accountserial)
						irc.SendMsg(msg)
					elseif ipmatch == false then
						InputConsole("msg [LuS] Warning, %s might be a nickspoofer! (IP doesn't match)", self:GetName())
						msg = string.format("[c=04][b][Protect][/b] Info: [u]%s[/u]'s Account IP: [u]%s[/u][/c]", self:GetName(), self.accountip)
						irc.SendMsg(msg)
					end
				end
