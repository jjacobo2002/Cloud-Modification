	...
	if Message[1] == "!commands" or Message[1] == "!help" then
		...
		if Message[1] == "!premiumhelp" then
			if user[pID]:HasAccess(1) then
				InputConsole("pamsg %d Premium Commands: !vipgamble, !bonuscreds, !tankbots.", pID)	
		...
		if Message[1] == "!mischelp" then
			InputConsole("pamsg %d Commands: !gethp <player>, !scripts <player>, !tdonate <credits>, !sounds, !killme", pID)
		...
		if Message[1] == "!buyhelp" then
			...
			if Message[1] == "!bots" then
				if user[pID]:HasAccess(1) then
					...
					InputConsole("pamsg %d Commands: !rambot (3000).", pID)
				else
					InputConsole("ppage %d You do not have access to this command.", pID)
				end
				return 1
			...
			if Message[1] == "!build" then
				if user[pID]:HasAccess(1) then
					if Get_Team(pID) == 1 then
						InputConsole("pamsg %d GDI Commands: !gun (500), !g (600), !it (2000), !g2 (2500), !tg (3000), !gneg silo (12000), !agt (35000).", pID)
					elseif Get_Team(pID) == 0 then
						InputConsole("pamsg %d Nod Commands: !tail (400), !t (500), !t2 (2500), !nneg silo (12000), !obby/!obelisk (35000).", pID)
					end
			...
			if Message[1] == "!chars" then
				if user[pID]:HasAccess(1) then
					...
					InputConsole("pamsg %d Commands: !engie (<0), !sbh (1000), !hotwire (1500), !tech (1500), !jumpy (2000), !ralph (3000), !orphan (3000), !mrmutant (3000), !c4tech (3800), !prisoner (4500), !mrmoney (5000).", pID)
				else
					InputConsole("ppage %d You do not have access to this command.", pID)
				end
				return 1
			...
			if Message[1] == "!upgrades" then
				if user[pID]:HasAccess(1) then
					InputConsole("pamsg %d Commands: !up (1000), !upv (2000), !weapons, !regens, !suits.", pID)
				else
					InputConsole("ppage %d You do not have access to this command.", pID)
				end
				return 1
