...
function Kick(kID, ByID, Reason)
	nickname = user[kID]:GetName()
	query = string.format("INSERT INTO kick_log VALUES(%d, '%s', '%s', '%s', '%s'", os.time(), nickname, user[kID]:GetCurrentIP(), user[ByID]:GetName(), Reason)
	db:Query("", "", query)
	InputConsole("kick %d", kID)
	InputConsole("allow %s", nickname)
	InputConsole("msg [LuS] %s has been qkicked by %s for: %s", nickname, user[ByID]:GetName(), Reason)
end
