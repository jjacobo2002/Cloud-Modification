		...
		if Are_Teams_Even() == true then
			...
			for pID, value in pairs(user) do
				if team == Get_Team(pID) then
					...
					if user[pID]:IsVIP() then
						...
						irc.SendMsg(string.format("[Debug] %s - Is a VIP - Removed 1 NP - Total: %f", name, np[name].np))
					end
						
					scripts = tonumber(user[pID]:GetScripts())
					if scripts ~= nil and scripts >= 3 then
						np[name].np = np[name].np - 1
						irc.SendMsg(string.format("[Debug] %s - Has scripts %s - Removed 1 NP - Total: %f", name, scripts, np[name].np))
					end
				end
			end
			...
			while x ~= 0 do
				...
				if np[name] ~= nil then
					...
					irc.SendMsg(string.format("[Debug] %s - Spend %d credits - Added %f NP - Total: %f", name, lastspend, npadd, np[name].np))
				end
				moneyspenders[x] = nil
				x = table.maxn(moneyspenders)
			end
			...
			while x ~= 0 do
				...
				if np[name] ~= nil then
					...
					irc.SendMsg(string.format("[Debug] %s - Jointime: %d - Added %f NP - Total: %f", name, lastjoin, npadd, np[name].np))
				end
				jointimes[x] = nil
				x = table.maxn(jointimes)
			end
			...
			while Are_Teams_Even() ~= true do
				...
				if user[tID] ~= nil then
					...
					irc.SendMsg(string.format("[Debug] %s - NP Points: %f - Changed", user[tID]:GetName(), nporder[x].np))
				end
				nporder[x] = nil
			end
		end
