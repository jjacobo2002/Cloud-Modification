...
function Player:Login(pw)
	...
	if self:IsDisabled() == false then
		...
		elseif self:IsBanned() then
			...
			sql = string.format("SELECT COUNT(*) FROM users WHERE nickname LIKE '%s'", self:GetName())
			db:Query(self.pID, "query_UserCheck", sql)
		end
	end
end
...
function Player:AssignData(data)
	-- Making sure some values are numbers.	--
	data[1] = tonumber(data[1])		-- DB ID
	data[3] = tonumber(data[3])		-- Access
	data[4] = tonumber(data[4])		-- Credits
	data[5] = tonumber(data[5])		-- Recs
	data[6] = tonumber(data[6])		-- DP
	data[7] = tonumber(data[7])		-- Loti
	data[8] = tonumber(data[8])		-- Jointime
	data[9] = tonumber(data[9])		-- Onlinetime
	data[10] = tonumber(data[10])	-- Last Present
	data[11] = tonumber(data[11])	-- Last VIP Update
	data[12] = tonumber(data[12])	-- VIP
	data[13] = tonumber(data[13])	-- VIP Months
	data[14] = tonumber(data[14])	-- VIP Subscription
	data[18] = tonumber(data[18])	-- Protection
	data[19] = tonumber(data[19])	-- Auto Login
	data[20] = tonumber(data[20])	-- Sound
	
	-- If it isn't a new player / has Serial & IP, then	do warn/protect/autologin stuff	--
	if data[16] ~= nil and data[17] ~= nil then
		-- Pre-loading certain values	--
		self.autologin = data[19]
		self.protection = data[18]
		self.accountserial = data[17]
		self.accountip = data[16]
		
		ipmatch = mf.CompareIP(self.accountip, self.currentip)
		serialmatch = mf.CompareString(self.accountserial, self.currentserial)
	...
	if self.login == false then
		if data[15] == nil or data[15] == self.password then
			self.login = true
	...
	if self.login == true then
		self.dbid = data[1]
		self.access = data[3]
		self.bankcredits = data[4]
		self.recs = data[5]
		self.dp = data[6]
		self.loti = data[7]
		self.jointime = data[8]
		self.onlinetime = data[9]
		self.last_present = data[10]
		self.last_vip_update = data[11]
		self.vip = data[12]
		self.vip_months = data[13]
		self.vip_subscribe = data[14]
		self.password = data[15]
		self.accountip = data[16]
		self.accountserial = data[17]
		self.protection = data[18]
		self.autologin = data[19]
		...
		InputConsole("ppage %d Type !accounthelp for help with your account.", self.pID)
	end
end
...
function Player:SetModPower(State, Died)
	if State == true and self.modpower == false then		
		...
		if self:HasAccess(4) then
			pObj = Get_GameObj(self.pID)
			Grant_Powerup(pObj, "POW_Mobius_Shield")
		end
