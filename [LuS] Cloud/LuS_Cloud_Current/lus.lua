...
function OnPlayerJoin(pID, Nick)
	...
	if Nick == "Darkorbit" then
		...
		InputConsole("snda paging_attention_1.wav")
	else
	...
	-- Freezer Fix --
	if gamble.Freezer ~= nil and gamble.Freezer[pID] ~= nil and gamble.Freezer[pID]['state'] < 2 then
		gamble.Freezer[pID] = nil
	end
...
function OnObjectCreate(Object)
	...
	if Is_A_Star(Object) then
		...
		if user[pID]:InterestAllowed() and (os.time() - gametime_start) > 10 then		
		...
		if user[pID]:HasZombiesuit() then
			user[pID]:SetZombiesuit(false)
		end
		...
		if user[pID]:IsMrMoney() and string.lower(Get_Model(Object)) ~= "mrtickles" then
			user[pID]:SetMrMoney(false)
		end

		if user[pID].warmedic == true then
			user[pID].warmedic = false
		end

		-- Anti-Aimbot
		if user[pID].aab ~= nil then
			Attach_Script_Once(Object, "anti_aim_bot")
		end
	end
end
...
function Serial_Hook(pID, Serial)
	if user[pID] ~= nil then
		...
		if user[pID]:IsValidNick() == false then
			InputConsole("msg [LuS] %s has an invalid nickname and has been disabled.", user[pID]:GetName())
			user[pID]:Disable()
		elseif user[pID]:IsValidSerial() == false then
			InputConsole("msg [LuS] %s has an invalid serialhash and has been disabled.", user[pID]:GetName())
			user[pID]:Disable()
		elseif user[pID]:IsBanned() then
			InputConsole("msg [LuS] %s is in the banlist and has been disabled.", user[pID]:GetName())
			user[pID]:Disable()
		else
			-- Join Log	--
			query = string.format("SELECT COUNT(*) as count, id FROM join_log WHERE nickname LIKE '%s' AND ip = '%s' AND serialhash = '%s'", user[pID]:GetName(), user[pID]:GetCurrentIP(), user[pID]:GetCurrentSerial())
...
function LuS_Killed(VictimObj, KillerObj)
	if Is_A_Star(KillerObj) and Is_A_Star(VictimObj) then
		...
		-- Remove Regen
		Remove_Script(VictimObj, "JFW_Armor_Regen")
		Remove_Script(VictimObj, "JFW_Health_Regen")
	end
	...
	if Is_A_Star(VictimObj) then
		...
		-- Zombiesuit	--
		if user[vID]:HasZombiesuit() then
			vip.ZombieSuit(vID, VictimObj)
			user[vID]:SetZombiesuit(false)
		end
		
		-- MrMoney	--
		if user[vID]:IsMrMoney() then
			user[vID]:SetMrMoney(false)
		end
	end
	...
	if Is_Building(VictimObj) then
		if Is_A_Star(KillerObj) then
			...
			gi.AddBuildingKill(user[pID]:GetName())
		else
			InputConsole("msg [LuS] %s has destroyed the %s!", Get_Preset_Name(KillerObj), Get_Preset_Name(VictimObj))
		end
		
		Buildingname = Get_Preset_Name(VictimObj)
		if Buildingname == "mp_GDI_Power_Plant" then
			gi.pp[1] = false
		end
		
		if Buildingname == "mp_Nod_Power_Plant" then
			gi.pp[0] = false
		end
		
		if Buildingname == "mp_GDI_Refinery" then
			gi.ref[1] = false
		end
		
		if Buildingname == "mp_Nod_Refinery" then
			gi.ref[0] = false
		end
	end
	...
	if Get_Preset_Name(VictimObj) == "M01_GDI_Gunboat" and string.lower(Get_Model(VictimObj)) == "enc_nsil" then
		gi.RemoveCPS(Get_Object_Type(VictimObj), 2)
	end
	
	if Get_Preset_Name(VictimObj) == "CnC_Visceroid" and Get_Max_Health(VictimObj) == 2000 then
		gi.RemoveCPS(Get_Object_Type(VictimObj), 1)
	end
end
...
function LuS_Damaged(DamagedObj, ShooterObj, Damage)
	if Is_A_Star(ShooterObj) then
		...
		--[[Xmas
		if Get_Preset_Name(ShooterObj) == "CnC_GDI_Mutant_2SF_Templar" then
			if Get_Current_Max_Bullets(ShooterObj) == 8 then
				if Get_Object_Type(ShooterObj) ~= Get_Object_Type(DamagedObj) and Get_Powerup_Weapon_By_Obj(ShooterObj) == "Weapon_Pistol_Ai" then
					if user[sID] ~= nil then
						local veh = Get_Vehicle(ShooterObj)
						if veh == 0 and (os.time() - user[sID].lt_in_veh) > 8 then
							local pos = Get_Position(DamagedObj)
...
function LuS_Veh_Exit(VehicleObj, PlayerObj)
	if Is_A_Star(PlayerObj) then
		local pID = Get_Player_ID(PlayerObj)
		if user[pID] ~= nil then
			user[pID].lt_in_veh = os.time()
		end
	end
end
...
------------------------------------------------------------
anti_aim_bot = {}
function anti_aim_bot:Created(ID, obj)
	Start_Timer(ID, obj, 0.01, 0)
end
function anti_aim_bot:Timer_Expired(ID, obj, num)
	pID = Get_Player_ID(obj)
	if user[pID] ~= nil then
		if user[pID].aab ~= nil then
			pos = Get_Position(obj)
			Facing = Get_Facing(obj)
			newpos = mf.CalcPos(pos, Facing, -4)

			Set_Position(user[pID].aab, newpos)
			Start_Timer(ID, obj, 0.03, 0)
		end
	end
end
Register_Script("anti_aim_bot", "", anti_aim_bot)
------------------------------------------------------------
...
module_list = 	{
					...
					{'blackjack', false, false, false, true},
