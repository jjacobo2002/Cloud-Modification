	...
	if Message[1] == "!commands" or Message[1] == "!help" then
		...
		if Message[1] == "!premiumhelp" then
			if user[pID]:HasAccess(1) then
				InputConsole("pamsg %d Premium Commands: !vipgamble, !bonuscreds, !vipvehregen, !upx10, !hummerbot, !buggybot, !elitequad, !shockerbot, !ae, !g3, !t3, !ambush <player>, !zs, !ago, !hh, !d6.", pID)	
		...
		if Message[1] == "!modhelp" then
			if user[pID]:HasAccess(2) then
				...
				if user[pID]:HasAccess(4) then
					...
					InputConsole("pamsg %d Commands: !addrecs <player> <recs>, !removerecs <player> <recs>, !setrecs <player> <recs>, !refund <player> <credits>, !refundall <credits>", pID)
		...
		if Message[1] == "!buyhelp" then
			...
			if Message[1] == "!updates" then
				...
				--InputConsole("pamsg %d EVERYONE: !agtgun (5000) !shockergun (5000). Changed repair commands. Updated September 3rd", pID)		
				InputConsole("pamsg %d VIP: !d6 + !hh, EVERYONE: !pbot !lcgbot !blackjackhelp. Changed repair commands. Updated July 21st", pID)		
			...
			if Message[1] == "!bots" then
				if user[pID]:HasAccess(1) then
					if Get_Team(pID) == 1 then
						InputConsole("pamsg %d GDI Commands: !gbot (1000), !grbot (1800), !pbot (2600), !mobot (2500), !rgbot (2800), !hotbot (3500), !gdicommander (4000), !gdiarmy (4000), !sbot (5000).", pID)
					elseif Get_Team(pID) == 0 then
						InputConsole("pamsg %d Nod Commands: !nbot (1000), !nrbot (1800), !lcgbot (2600), !mebot (2500), !rnbot (2800), !techbot (3500), !nodcommander (4000), !nodarmy (4000), !rbot (5000).", pID)
					end
					InputConsole("pamsg %d Commands: !rambot (3000), !buyshockerbot (10 Recs).", pID)
					if user[pID]:IsVIP() then
						InputConsole("pamsg %d Premium Commands: GDI: !deadsix (4250) Nod: !helpinghand (4250) !elitesquad (10000), !shockerbot (3000), !ambush <player> (6000)", pID)
					end
				else
					InputConsole("ppage %d You do not have access to this command.", pID)
				end
			...
			if Message[1] == "!build" then
				if user[pID]:HasAccess(1) then
					...
					if user[pID]:IsVIP() then
						if Get_Team(pID) == 1 then
							InputConsole("pamsg %d Premium Commands: !g3 (4500), !ae (16000), !ago (45 000).", pID)
						else
							InputConsole("pamsg %d Premium Commands: !t3 (4500), !ae (16000), !ago (45 000).", pID)
						end
					end
				else
					InputConsole("ppage %d You do not have access to this command.", pID)
			...
			if Message[1] == "!chars" or Message[1] == "!char" then
				if user[pID]:HasAccess(1) then
					InputConsole("pamsg %d Commands: !yuri (5000), !rav (6500), !darkorbit (8000), !petrova (9000), !reddwire (12 750), !aqollo (20-55k), !assassin (15 500), !fsfox (20 000), !spy123321/!spy (25 000).", pID)
			...
			if Message[1] == "!blackjackhelp" then
				if user[pID]:HasAccess(1) then
					InputConsole("pamsg %d The basic premise of the game is that you want to have a hand value that is closer to 21 than that of the dealer, without going over 21.", pID)
					InputConsole("pamsg %d !blackjack <bet>, !blackjackstats, !mycards - shows your current hand, !hit - gives you additional card, !stay - you receive no more cards and end the hand,", pID)
				else
					InputConsole("ppage %d You do not have access to this command.", pID)
				end
				return 1
			...
			if Message[1] == "!gambles" then
				if user[pID]:HasAccess(1) then
					InputConsole("pamsg %d Commands: !lowgamble (500), !chancegamble (1500), !medgamble (3500), !highgamble (10000), !blackjack <bet>, !blackjackhelp, !freezerhelp.", pID)
					if user[pID]:IsVIP() then
