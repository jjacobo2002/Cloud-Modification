...
function FindPlayer(Result, Name)
	...
	while CurID <= MaxID do
		...
		if user[CurID] ~= nil then
			...
			elseif string.find(string.lower(CurName), string.lower(Name), 1, true) ~= nil then
...
function Get_Distance(Obj1, Obj2, ZAxis)
	...
	if ZAxis then
		...
		Distance = math.sqrt((pos.X - pos2.X)^2+(pos.Y - pos2.Y)^2)
	end
...
function CalcPos(pos, Facing, Distance, Add)
	newpos = {}
	newpos.X = pos.X + (Distance*math.cos(Facing*(math.pi / 180)))
	newpos.Y = pos.Y + (Distance*math.sin(Facing*(math.pi / 180)))
	newpos.Z = pos.Z
	
	if type(Add) == "table" then
		for k, v in pairs(Add) do
			if newpos[k] ~= nil then
				newpos[k] = newpos[k] + v
			end
		end
	end
...
function FormatDate(timestamp)
	...
	if tt['day'] == 1 or tt['day'] == 21 or tt['day'] == 31 then
		dayadd = "st"
	elseif tt['day'] == 2 or tt['day'] == 22 then
		dayadd = "nd"
	elseif tt['day'] == 3 or tt['day'] == 23 then
		dayadd = "rd"
	else
		dayadd = "th"
	end

	return string.format("%d%s %s %s", tt['day'], dayadd, os.date("%B", timestamp), tt['year'])
end
