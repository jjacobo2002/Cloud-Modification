...
function Command(pID, Message)
	...
	--[[if Message[1] == "!agtgun" then
		if user[pID]:HasAccess(1) then
			price = 5000
			if Get_Money(pID) >= price then 
				InputConsole("msg %s has bought an AGT Gun!", user[pID]:GetName())
				Set_Money(pID, Get_Money(pID)-price) 
				Grant_Powerup(Get_GameObj(pID), "Weapon_AGT_Missile") 
				...
				InputConsole("ppage %d You need %d credits for an AGT Gun", pID, price)
			end
		else
			InputConsole("ppage %d You do not have access to this command.", pID)
		end
	...
	if Message[1] == "!shockergun" then
		if user[pID]:HasAccess(1) then
			price = 5000
			if Get_Money(pID) >= price then 
				InputConsole("msg %s has bought a Shocker gun!", user[pID]:GetName())
				Set_Money(pID, Get_Money(pID)-price) 
				Grant_Powerup(Get_GameObj(pID), "Weapon_Ceiling_Gun_M11") 
				...
				InputConsole("ppage %d You need %d credits for a shocker gun", pID, price)
			end
		else
			InputConsole("ppage %d You do not have access to this command.", pID)
		end
	...
	if Message[1] == "!shockergun" then
		if user[pID]:HasAccess(1) then
			...
			if Get_Money(pID) >= price then 
				InputConsole("msg %s has bought a Shocker gun!", user[pID]:GetName())
				Set_Money(pID, Get_Money(pID)-price) 
				Grant_Powerup(Get_GameObj(pID), "POW_Adrenaline_Syringe") 
	...
	if Message[1] == "!agtpack" then
		if user[pID]:HasAccess(1) then
			price = 3000
			if Get_Money(pID) >= price then 
				InputConsole("msg %s has bought an agt pack!", user[pID]:GetName())
				Set_Money(pID, Get_Money(pID)-price) 
				Grant_Powerup(Get_GameObj(pID), "POW_Tissue_Nanites")
				...
				InputConsole("ppage %d You need %d credits for an agt pack!", pID, price)
			end
		else
			InputConsole("ppage %d You do not have access to this command.", pID)
		end
	...
	if Message[1] == "!weakrepair" or Message[1] == "!wrep" then
		if user[pID]:HasAccess(1) then
			price = 200
			if Get_Money(pID) >= price then 
	...
	if Message[1] == "!megarepair" or Message[1] == "!mrep" or Message[1] == "!repair" then
		if user[pID]:HasAccess(1) then
			price = 1500
			if Get_Money(pID) >= price then 
		...
		if user[pID]:HasAccess(1) then
			...
			if scripts == nil or scripts == false or scripts < 2.9 then
				...
				if Is_Stealth(pObj) then
					...
					InputConsole("ppage %d You reactivated your stealth-suit.", pID)
				else
					price = 3000
					if Get_Money(pID) < price then
						if Get_Team(pID) == 1 then
