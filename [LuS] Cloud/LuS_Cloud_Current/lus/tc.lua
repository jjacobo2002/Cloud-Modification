...
function StartBalance()
	team_to_change = Are_Teams_Even()
	if team_to_change ~= true then
		if autobalance == false then
			...
			if team_to_change == 0 then
				teamname = "Nod"
			elseif team_to_change == 1 then
				teamname = "GDI"
			else
				teamname = "?"
			end
...
function BalanceNow()
	if os.time() - start_time > 60 then
		...
		if Are_Teams_Even() == true then
			...
			-- Remove Np	--
			for pID, value in pairs(user) do
				if team_to_change == Get_Team(pID) then
					local name = user[pID]:GetName()
					
					-- Remove Np if VIP	--
					if user[pID]:IsVIP() then
						np[name].np = np[name].np - 0.5
						--irc.SendMsg(string.format("[b][Debug][/b] %s - Is a VIP - Removed 0.5 NP - Total: %f", name, np[name].np))
					end
						
					-- Remove Np if Scripts > 3	--
					local scripts = tonumber(user[pID]:GetScripts())
					...
					-- Remove Np based on buildings killed
					local bk = gi.GetBuildingKill(name)
					if bk ~= nil and bk > 0 then
						np[name].np = np[name].np - (bk*2)
						--irc.SendMsg(string.format("[b][Debug][/b] %s - Killed %d buildings - Removed %d NP - Total: %f", name, bk, (bk*2), np[name].np))
					end
				end
			end
				
			-- Add NP based on Money Spend
			...
			while x ~= 0 do
				...
				mID = mf.FindPlayer("ID", name)
				if np[name] ~= nil and user[mID] ~= nil and team_to_change == Get_Team(mID) then
