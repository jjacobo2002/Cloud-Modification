...
function Command(pID, Message)
	...
	if Message[1] == "!helpinghand" or Message[1] == "!hh" then
		if user[pID]:HasAccess(1) then
			if user[pID]:IsVIP() then
				if Get_Team(pID) == 0 then
					...
					if Get_Money(pID) < price then
						...
						Bots = 	{ 	"Nod_RocketSoldier_2SF_AutoRifle", "Nod_Minigunner_2SF", "Nod_FlameThrower_2SF", "Nod_RocketSoldier_2SF_AutoRifle", "Nod_Minigunner_2SF", "Nod_FlameThrower_2SF"
							}


						Scripts = { M01_Hunt_The_Player_JDG="",lus_damaged="", lus_poked="", lus_killed="" }
						bots.MultiSpawn(Get_GameObj(pID), Bots, table.maxn(Bots), 2, 0.1, false, Scripts)      --Nod_RocketSoldier_2SF_AutoRifle,Nod_Minigunner_2SF,Nod_FlameThrower_2SF
	...
	if Message[1] == "!deadsix" or Message[1] == "!d6" then
		if user[pID]:HasAccess(1) then
			if user[pID]:IsVIP() then
				if Get_Team(pID) == 1 then
					price = 4250
					if Get_Money(pID) < price then
						...
						Bots = 	{  	"GDI_MiniGunner_2SF", "GDI_RocketSoldier_2SF", "GDI_Grenadier_2SF", "GDI_MiniGunner_3Boss", "GDI_RocketSoldier_0"
							}

						Scripts2 = { M01_Hunt_The_Player_JDG="", z_Set_Team=2, RA_Infantry_Spy="",lus_damaged="", lus_poked="", lus_killed="" }
						Scripts = { M01_Hunt_The_Player_JDG="",lus_damaged="", lus_poked="", lus_killed="" }
						bots.MultiSpawn(Get_GameObj(pID), Bots, table.maxn(Bots), 3, 0.1, false, Scripts)     
						bots.MultiSpawn(Get_GameObj(pID), "GDI_Engineer_2SF", 1, 2, 0.1, false, Scripts2) 
						InputConsole("cmsg 0,250,250 %s has bought bots!", user[pID]:GetName())
		...
		if user[pID]:HasAccess(1) then
			if user[pID]:IsVIP() then
				...
				if Get_Money(pID) < price then
					...
					bots.MultiSpawn(Get_GameObj(pID), "civ_lab_tech_01", 2, 2, 0.5, false, { z_Set_Team=team, lus_damaged="", lus_poked="", lus_killed="", changemodel="dino" })
